		...
		<view :class="[direction==='column'?'uni-steps__column':'uni-steps__row']">
			<view :class="[direction==='column'?'uni-steps__column-text-container':'uni-steps__row-text-container']">
				<view v-for="(item,index) in options" :key="index" :class="[direction==='column'?'uni-steps__column-text':'uni-steps__row-text']">
					<text v-if="item.title!='投诉失败'" :style="{color:index<=active?activeColor:deactiveColor}" :class="[direction==='column'?'uni-steps__column-title':'uni-steps__row-title']">{{item.title}}</text>
					<text v-if="item.title=='投诉失败'" :style="{color:index<=active?activeColor1:deactiveColor}" :class="[direction==='column'?'uni-steps__column-title':'uni-steps__row-title']">{{item.title}}</text>
					<text style="height: 40upx;" :class="[direction==='column'?'uni-steps__column-desc':'uni-steps__row-desc']"></text>
				...
				<view :class="[direction==='column'?'uni-steps__column-line-item':'uni-steps__row-line-item']" v-for="(item,index) in options"
				 :key="index">
					...
					<view :class="[direction==='column'?'uni-steps__column-check':'uni-steps__row-check']" v-if="index === active">
						<uni-icons v-if="item.title=='投诉失败'" :color="activeColor1" type="clear" size="20"></uni-icons>
						<uni-icons v-if="item.title!='投诉失败'" :color="activeColor" type="checkbox-filled" size="20"></uni-icons>
					</view>
					<!-- <view :class="[direction==='column'?'uni-steps__column-check':'uni-steps__row-check']" v-if="index === active&&">
